...
| M4 Provider Portfolio | done | Complete | Chatai (SSE), Yqcloud (plain stream), WeWordle (JSON) implemented with `httptest` suites; live endpoints inactive as of 2026-06-21 (Chatai DNS SERVFAIL, Yqcloud 405, WeWordle 404) — code and mocked tests satisfy the "three providers implemented or explicitly status-documented" gate. |
| M5 CLI | done | Complete | `cmd/gollmfree/main.go` with `chat`/`list`/`models` commands; `list`/`models` no-network tests pass; `chat` uses `Client.ChatCompletion`; `--model`, `--timeout`, `--race`, `--json` flags implemented. |
| M6 `gormes-agent` Integration | done | Complete | `llm.Client` interface inspected; `WithRegistry` option added to gollmfree; `NewGollmfreeClient()` adapter wired and live-tested end-to-end via navivox WebSocket. |
...
- **Next task:** T7.1 README quick start.
- **Reason:** M4–M6 are complete. M7 is the last gate before v0.1.0. README needs a working quick-start snippet, updated provider status table (PollinationsAI active; Chatai/Yqcloud/WeWordle inactive with reason), GoDoc pass, and CI green confirmation.
- **Prerequisite note:** Three of the four registered providers (Chatai, Yqcloud, WeWordle) have inactive live endpoints as of 2026-06-21. The release README must document their status honestly. Re-activation requires finding working endpoints and adding integration tests.
...
| 2026-06-21 | CLI uses standard `flag` for v0.1.0. | Keeps dependencies minimal; `cobra` not needed for four commands. | If command UX becomes complex enough to justify Cobra. |
| 2026-06-21 | Add `WithRegistry(*Registry) Option` to allow external callers to wire providers without accessing unexported fields. | `gormes-agent`'s `NewGollmfreeClient()` needs to register concrete providers at construction time; the unexported `registry` field was not accessible from outside the package. | If registry becomes a first-class public field or a builder pattern replaces functional options. |
| 2026-06-21 | Set `maxRetries=0` in `gormes-agent`'s `NewGollmfreeClient()`. | PollinationsAI allows only 1 concurrent anonymous request per IP; immediate retries on 429 occupy the same queue slot and guarantee further 429s. One attempt per turn lets the queue recover naturally (~30s gap). | If a back-off/jitter retry strategy is added to the gollmfree client. |
| 2026-06-21 | Register only PollinationsAI in `NewGollmfreeClient()` until working endpoints are confirmed for Chatai/Yqcloud/WeWordle. | Live endpoint investigation (2026-06-21): `chatai.ren` DNS SERVFAIL; `chat9.yqcloud.top` 405 on all POST paths; `wewordle.org/gptapi/v1/en/trial` 404. Dead providers add latency without adding resilience. | Re-add when working endpoints are validated by integration tests. |
...
| `gormes-agent` LLM interface unknown | resolved | `internal/llm.Client` interface inspected; `WithRegistry` option added; `NewGollmfreeClient()` adapter wired and live-tested. | — |
| Live provider viability unknown | partially resolved | PollinationsAI: active, anonymous, rate-limit 1 req/IP queue. Chatai: DNS SERVFAIL (2026-06-21). Yqcloud `chat9.yqcloud.top`: domain resolves but 405 on all POST paths (2026-06-21). WeWordle `/gptapi/v1/en/trial`: 404 (2026-06-21). Need new endpoint research before re-activating. | Release README provider table; re-activation. |
...
| 2026-06-21 | T4.2 Chatai provider | `go test ./providers -run TestChatai` failed to build because `NewChatai` and `chataiRequest` were undefined. | `go test ./providers -run TestChatai` — all 5 tests pass; SSE parsing, non-data line skip, non-2xx error, context cancellation, stream emulation. `go test ./...` green. | `providers/chatai.go`, `providers/chatai_test.go` | T4.3 Yqcloud |
| 2026-06-21 | T4.3 Yqcloud provider | `go test ./providers -run TestYqcloud` failed to build because `NewYqcloud` and `yqcloudRequest` were undefined. | `go test ./providers -run TestYqcloud` — all 5 tests pass; prompt extraction, last-user-message selection, non-2xx error, context cancellation, stream emulation. `go test ./...` green. | `providers/yqcloud.go`, `providers/yqcloud_test.go` | T4.4 WeWordle |
| 2026-06-21 | T4.4 WeWordle provider | `go test ./providers -run TestWeWordle` failed to build because `NewWeWordle` and `wewordleRequest` were undefined. | `go test ./providers -run TestWeWordle` — all 6 tests pass; subscriber-shape POST, message role preservation, malformed JSON, non-2xx, context cancellation, stream emulation. `go test ./...` green. | `providers/wewordle.go`, `providers/wewordle_test.go` | T5.1 CLI |
| 2026-06-21 | Live endpoint investigation (post-M4) | All three new providers fail live. `chatai.ren` DNS SERVFAIL. `chat9.yqcloud.top` 405 on all POST paths (domain resolves, OPTIONS/HEAD succeed). `wewordle.org/gptapi/v1/en/trial` 404 (domain resolves, Express route missing). PollinationsAI 429 when retried immediately (queue full, 1 req/IP). | Documented in decision log; `NewGollmfreeClient()` reverted to PollinationsAI-only; `maxRetries` set to 0. | `internal/llm/gollmfree_client.go` in gormes-agent | T7.1 (endpoint re-research before release) |
| 2026-06-21 | T5.1–T5.4 CLI (`chat`/`list`/`models`) | `go test ./cmd/gollmfree` failed to build because `cmd/gollmfree/` did not exist. | `go test ./cmd/gollmfree -run 'TestRunNoArgs\|TestCmdList\|TestCmdModels\|TestDefaultRegistry'` — all 6 tests pass; no-network `list`/`models`; usage error returns exit 2; default registry contains all four providers; `auto` alias present. `go test ./...` green. | `cmd/gollmfree/main.go`, `cmd/gollmfree/main_test.go` | T6.1 gormes-agent integration |
| 2026-06-21 | T6.1 + T6.2 gormes-agent integration | `gormes-agent` used unexported `registry` field; `NewGollmfreeClient()` could not wire providers externally. | `WithRegistry(*Registry) Option` added to `options.go`; `client.go` uses `registryOverride` when non-nil; `NewGollmfreeClient()` builds a registry, wraps it, and the navivox WebSocket test confirmed end-to-end reply ("2 + 2 = 4"). `go test ./...` green in both repos. | `options.go`, `client.go` in gollmfree; `internal/llm/gollmfree_client.go` in gormes-agent | T7.1 |
